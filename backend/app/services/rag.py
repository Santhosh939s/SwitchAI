...
RAG_KNOWLEDGE_BASE: List[Dict[str, str]] = [
    ...
    {"topic": "switch_ai", "keywords": "switch ai switchai platform what is switch ai one memory many ai models workspace", "response": "SwitchAI is a unified AI workspace and multi-provider platform that decouples conversation memory from underlying AI models (Google Gemini, OpenAI GPT, Anthropic Claude, DeepSeek, and Local AI). It maintains ONE shared memory bank and Context Passport across model switches with zero context loss and zero token waste."},
