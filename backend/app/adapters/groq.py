...
class GroqAdapter(ProviderAdapter):
    ...
    def generate(self, context_package: ContextPackage, api_key: str, model: Optional[str] = None) -> ProviderResponse:
        ...
        if not api_key:
            raise ValueError("Groq API key is missing. Please connect your Groq API key in Settings -> Providers.")
        ...
        if api_key.startswith("mock_"):
            return ProviderResponse(
                content=f"Hello! I am Groq LPU ({model_name}). I am active with your unified shared memory.",
                provider="groq",
                model=model_name,
                latency_ms=35.0,
        ...
        with httpx.Client(timeout=25.0) as client:
            ...
            if res.status_code == 404:
                raise ValueError(f"Selected Groq model '{model_name}' is unavailable. Please select another model.")
    ...
    async def stream(self, context_package: ContextPackage, api_key: str, model: Optional[str] = None) -> AsyncGenerator[str, None]:
        yield "[Groq Stream] High speed stream active."

    def normalize_error(self, error: Exception) -> NormalizedError:
        err_msg = str(error)
        err_lower = err_msg.lower()
        if "rate" in err_lower or "429" in err_msg:
            return NormalizedError(code="RATE_LIMIT", message="Groq rate limit reached", retry_after_seconds=3, is_retryable=True)
    ...
    def get_health(self, api_key: str) -> ProviderHealth:
        ...
        if not is_valid:
            return ProviderHealth(provider="groq", is_healthy=False, status_message="Credential Validation Failed", discovered_models=self.DEFAULT_MODELS)
